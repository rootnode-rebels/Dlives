# Dlives — Technical Architecture & Complete Application Manual

**Dlives** is a production-grade, native Windows 10/11 desktop widget built with Python 3.14, PyQt6, and Win32 C-API interop. It brings an interactive, hardware-accelerated dark OLED glass dynamic island to Windows with live system telemetry, control sliders, media transport, scheduled alarms with audio chimes, calendar storage, task management, quick notes, live UI customization, and system tray integration.
...
├── storage_manager.py         # tasks.json, notes/, calendar.json & alarms.json persistence
├── ui_components.py           # Mini-taskbar pill header & interactive dashboard tabs
├── full_app_window.py         # Resizable Dlives Customization Studio & modules window
├── requirements.txt           # Python dependency manifest
├── installer.iss              # Inno Setup Windows installer script
├── dist/
│   └── Dlives.exe             # Compiled standalone executable
```
...
- **Win32 C-API Interop**:
  ...
  - `winreg` for Windows startup autorun (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Dlives`).
- **Telemetry Libraries**:
...
- Automatically creates or deletes registry keys in `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Dlives` when `start_with_windows` is toggled.

### D. `storage_manager.py`
- Centralized read/write storage manager for `tasks.json`, `notes/`, `calendar.json`, and `alarms.json` in `%APPDATA%\Dlives\`.
...
8. **`QuickNotesTabWidget` (`📝 Notes`)**:
   - Scratchpad text editor connected to `textChanged` signal for instant keystroke auto-saving (`notes/`).
...
- **Toggle Visibility**: Right-click system tray icon -> Select **Toggle Dlives** or click tray icon.
...
```bash
python -m PyInstaller --noconsole --onefile --name "Dlives" main.py
```
Output binary will be located at:
`d:\projects\dlives\dist\Dlives.exe`
